...
export default async function ItemDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
            <div className="mb-6">
                {item.groups ? (
                    <Link href={`/groups/${item.group_id}`} className="text-sm text-text-tertiary hover:underline">
                        &larr; Back to {item.groups.name}
                    </Link>
                ) : (
                    <Link href="/items" className="text-sm text-text-tertiary hover:underline">
                        &larr; Back to Items
                    </Link>
                )}
            </div>
            ...
            <div className="bg-surface border border-border rounded-lg p-8 shadow-sm">
                ...
                <div className="space-y-6">
                    {item.description && (
                        <div>
                            <h3 className="text-sm font-medium text-text-tertiary uppercase tracking-wider mb-1">
                                Description
                            </h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-text-tertiary uppercase tracking-wider mb-1">Owner</h3>
                    ...
                    {activeBorrow && contact ? (
                        <div className="bg-primary-50 dark:bg-primary-900/30 p-4 rounded-lg border border-primary-200 dark:border-primary-800">
                            <h3 className="text-sm font-medium text-text-secondary mb-2">Currently Borrowed By</h3>
                            ...
                            {contact.email && <p className="text-sm text-text-secondary">Email: {contact.email}</p>}
                            {contact.phone && <p className="text-sm text-text-secondary">Phone: {contact.phone}</p>}
                    ...
                    {isOwner && borrowHistory.length > 0 && (
                        <div className="pt-6 border-t border-border">
                            <h3 className="text-sm font-medium text-text-secondary mb-3">Borrow History</h3>
                    ...
                    <div className="pt-6 border-t border-border">
                        {item.status === 'available' ? (
                            ...
                            <div className="text-center p-3 bg-elevated rounded-lg text-text-tertiary">
