...
export default function SidebarClientContent({ user }: SidebarClientContentProps) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-40 md:hidden flex items-center justify-between p-4 bg-[var(--bg-base)] border-b border-[var(--border)]">
        <button
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-screen w-64 bg-[var(--bg-base)] border-r border-[var(--border)] flex flex-col transition-transform duration-300 z-30 md:z-auto md:translate-x-0 ${
        ...
        <div className="border-t border-[var(--border)] p-4 space-y-4">
          ...
          {user ? (
            <div className="space-y-2">
              <div className="px-4 py-2 rounded-lg bg-[var(--bg-surface)]">
              ...
              <form action={async () => { await logout() }} className="w-full">
                <button
                  type="submit"
                  className="w-full px-4 py-2 text-sm font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-surface)] rounded-lg transition-colors"
                  onClick={() => setIsOpen(false)}
                >
                  Sign Out
                </button>
